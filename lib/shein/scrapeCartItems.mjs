/**
 * Shared Shein cart share scraper (Playwright). Used by Next.js API and Railway worker.
 * @param {{ cartShareUrl?: string }} body
 * @returns {Promise<{ status: number, body: object }>}
 */
export function jsonResult(data, status = 200) {
  return { status, body: data }
}

export async function scrapeCartItems(body) {
  ...
  try {
    if (!body || typeof body !== 'object') {
      return jsonResult({ error: 'Invalid request body' }, 400)
    }
